					...
					<el-tab-pane label="敏感词管理" name="menu">
						...
						<div class="smart-widget-body">
							<el-table :data="labelMenus" v-loading="labelsLoading" border style="width: 100%">
								...
								<el-table-column :show-overflow-tooltip="true" prop="name" label="敏感词名">
								</el-table-column>
...
export default {
	...
	data () {
		return {
			...
			labelsupRules: {
				name: [
					{required: true, message: '敏感词名不能为空', trigger: 'blur'},
				],
			},
			labelMenuSearch:{
				name:''
	...
	methods: {
		...
		getLabels () {
			...
			this.$axios.post('/sensitivewords').then(response => {
		...
		// 添加敏感词
		addLabels () {
			this.$axios.post('/sensitivewords/add', this.labelsup).then(response => {
				const {status, data, message} = response.data
				if(status !== 0) return this.$message.error(status);
				this.$message.success('敏感词创建成功');
		...
		// 更新敏感词
		updateLabels () {
			this.$axios.post(`/sensitivewords/edit/`, this.labelsup).then(response => {
				const {status, data, message} = response.data
				if(status !== 0) return this.$message.error(status);
		...
		// 删除敏感词
		deleteLabels (id) {
			this.$axios.post(`/sensitivewords/delete/${id}`).then(response => {
				const {status, data, message} = response.data
				if(status !== 0) return this.$message.error(status);
				this.getLabels()
		...
		rowDelete (id) {
			this.$confirm('此操作将删除该敏感词信息, 是否继续?', '删除敏感词', {
				type: 'warning',
				center: true
			}).then(() => {
